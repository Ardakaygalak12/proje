...
import { ImageAnalysis, LanguageCode } from "../types";

const API_KEY = process.env.API_KEY || 'AIzaSyAAY-nW4x5mzgb7l1UkGN33JsACV0TlEUI';
...
export class GeminiService {
  ...
  async analyzeImage(base64Image: string, mimeType: string, langName: string): Promise<ImageAnalysis> {
    const response = await this.ai.models.generateContent({
      model: 'gemini-3-flash-preview',
      contents: [
        {
          parts: [
            { text: `Analyze this image in extreme detail. Provide a concise summary (max 3 sentences), and a list of specific details. 
  ...
  getLiveSession(callbacks: any, systemInstruction: string, voiceName: string = 'Zephyr') {
